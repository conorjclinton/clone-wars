...
function placeholderBackground(ctx, width, height, time) { ctx.fillStyle = '#29133f'; ctx.fillRect(0, 0, width, height); }
function placeholderGround(ctx, width, height, groundHeight, offset) { ctx.fillStyle = '#24152c'; ctx.fillRect(0, height - groundHeight, width, groundHeight); }
function placeholderBird(ctx, x, y, size, velocity) { ctx.fillStyle = '#f6d34a'; ctx.fillRect(x - size / 2, y - size / 2, size, size); }
function placeholderPipe(ctx, x, gapTop, gapBottom, pipeWidth, height) { ctx.fillStyle = '#6d244c'; ctx.fillRect(x, 0, pipeWidth, gapTop); ctx.fillRect(x, gapBottom, pipeWidth, height - gapBottom); }
...
let pipesMade = 0;
let groundOffset = 0;
let lastGapTop = (CONFIG.canvasHeight - CONFIG.groundHeight) / 2 - CONFIG.pipeGap / 2;
let score = 0;
let checkpoint = 0;
let bestScore = 0;
...
function showReady() {
  overlay.hidden = false; overlayTitle.textContent = CONFIG.title;
...
function startGame() {
  ...
  pipes = []; pipesMade = 0; groundOffset = 0; lastGapTop = (CONFIG.canvasHeight - CONFIG.groundHeight) / 2 - CONFIG.pipeGap / 2;
  score = checkpoint; secondsSinceCrash = 0; overlay.hidden = true; play('flap');
}
function press() {
...
function frame(now) {
  const seconds = Math.min((now - previous) / 1000, 0.05); previous = now;
  if (state === 'playing') {
    velocity += CONFIG.gravity * seconds; y += velocity * seconds;
    ...
    if (y + half >= CONFIG.canvasHeight - CONFIG.groundHeight || y - half <= 0 || pipes.some((pipe) => birdX + half > pipe.x && birdX - half < pipe.x + CONFIG.pipeWidth && (y - half < pipe.gapTop || y + half > pipe.gapBottom))) crash();
  ...
  if (state === 'playing') {
    ctx.save(); ctx.font = 'bold 36px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.lineWidth = 5; ctx.strokeStyle = '#321b3d'; ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 18); ctx.fillStyle = '#fff'; ctx.fillText(String(score), CONFIG.canvasWidth / 2, 18); ctx.restore();
  }
